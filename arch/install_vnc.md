# use NoMachine (NX Protocol)
NoMachine 使用其專有的 NX 協議，在處理高動態畫面（如影片或動畫）時比 VNC 快得多，且對 Arch Linux 的支援非常成熟。

優點：自動處理解析度縮放，幾乎不會出現 0x0 錯誤；支援聲音傳輸與檔案共享。
安裝：
Arch: yay -S nomachine (從 AUR 安裝)。
Mac: 從 NoMachine 官網 下載安裝 Client。
注意：它在 X11 下表現最完美，但在現代 Wayland 環境中也比大多數 VNC 工具穩定。
...
```bash
# to solve: Protocol error: bad desktop size 0x0
#4. 調整 RealVNC 客戶端設定
#在某些情況下，客戶端的請求方式可能導致伺服器報錯：
#在 RealVNC Viewer 中，進入連線的 Properties (內容) > Options。
#確保將 Scaling 設定為 「Automatic」 或 「100%」。
#嘗試將 Encryption 設定為 「Prefer off」，以排除加密握手導致的解析度回報異常。
